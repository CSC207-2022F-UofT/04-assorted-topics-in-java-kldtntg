...
class DrivableMap {
     ...
     */
    public boolean addDrivable(String key, Drivable obj){
        if (!drivable_map.containsKey(key)){drivable_map.put(key, obj); return true;}
        return false;
    ...
    public boolean hasFasterThan(int speed){
        Collection<Drivable> returned = drivable_map.values();
        ArrayList<Drivable>listOfItems = new ArrayList<>(returned);
        for (Drivable item : listOfItems){
            if (item.getMaxSpeed() >= speed){return true;}
        }
        return false;
    }
    ...
    public List<Tradable> getTradable(){
        ArrayList<Tradable> returned = new ArrayList<>();
        for (Drivable item : drivable_map.values()){
            if (item instanceof Tradable){
                returned.add((Tradable) item);
            }
        }
        return returned;
